# 功能：按《殷维_简历(1).pdf》原结构出投递版 docx，只在工作经历和项目经历顶部加入正知。
# 更新：2026-08-30：不大改模块顺序和原条目写法；不改原职位名、日期、项目数字。

"""读取下方写死的原结构内容，输出 殷维_简历_原结构_正知增补.docx。"""
...
OUT = Path(__file__).resolve().parent / "殷维_简历_原结构_正知增补.docx"
...
def tight(p, before=0, after=2, line=1.05):
    p.paragraph_format.space_before = Pt(before)
    p.paragraph_format.space_after = Pt(after)
    p.paragraph_format.line_spacing = line


def add_center(doc, text, size, bold=False, after=2, color=None):
    p = doc.add_paragraph()
...
def add_line(doc, text, size=10.5, bold=False, before=0, after=2, color=None):
    p = doc.add_paragraph()
...
def add_job(doc, company, dates, title, bullets):
    p = doc.add_paragraph()
    tight(p, 6, 0)
    set_run_font(p.add_run(company), size=10.5, bold=True)
    set_run_font(p.add_run("    " + dates), size=10, color=GRAY)
    add_line(doc, title, size=10, bold=False, after=1, color=GRAY)
    for i, b in enumerate(bullets, start=1):
        add_line(doc, f"{i}、{b}", size=10.5, after=1)


def add_project(doc, name, dates, desc, duties, result):
    p = doc.add_paragraph()
    tight(p, 6, 1)
    set_run_font(p.add_run("● " + name), size=10.5, bold=True)
    set_run_font(p.add_run("    " + dates), size=10, color=GRAY)
    add_line(doc, "项目描述：" + desc, size=10.5, after=1)
    add_line(doc, "个人职责：", size=10.5, bold=True, after=1)
    for i, b in enumerate(duties, start=1):
        add_line(doc, f"{i}、{b}", size=10.5, after=1)
    add_line(doc, "项目业绩：" + result, size=10.5, after=2)
...
def main():
    ...
    add_center(doc, "殷  维", 18, bold=True, after=4, color=NAVY)
    add_center(
        doc,
        "性别：女    年龄：31岁    籍贯：湖北武汉    政治面貌：中共党员    身高：165cm",
        10,
        after=1,
        color=GRAY,
    )
    add_center(doc, "专业：金融    学历：硕士", 10, after=1, color=GRAY)
    add_center(
        doc,
        "手机：15671678098    邮箱：yw43@foxmail.com    地址：武汉市洪山区",
        10,
        after=2,
        color=GRAY,
    )
    add_center(doc, "求职意向：AI产品经理（交易创新方向）", 10.5, bold=True, after=4)
    ...
    add_line(doc, "2017.09-2019.06    华中科技大学    金融    硕士（全日制）", size=10.5, before=4, after=2)
    add_line(doc, "2013.09-2017.06    武汉科技大学    机械工程    本科（全日制）", size=10.5, after=2)
    ...
    for line in [
        ...
        "通过英语四级、英语六级（558），能快速阅读英文文献，英语交流能力良好",
        "熟练掌握 python 语言、matlab、wind 等软件，能使用 wind 结合 excel 进行金融数据分析",
        "基于股利贴现模型和量化分析，完成硕士毕业论文《医药行业上市公司价值评估研究--以云南白药为例》",
        "熟练使用 claude、cursor、Coze 等 AI 工具进行辅助编程，了解 MCP 基本用法，能搭建 AI agent team 协同任务处理，提高效率；能把投研任务拆成采集、校验、失败换源，不把模型输出当成下单指令",
        "近期：Xpert 金融 Skill 评测、TalentsAI Agentic Coding × 金融已过审（远程交付，非全职经历）",
    ]:
        add_line(doc, line, size=10.5, after=2)
