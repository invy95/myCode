...
脚本在本 skill 目录下。用当前 `SKILL.md` 所在目录作为 `SKILL_ROOT`：

```bash
python3 "$SKILL_ROOT/scripts/qimen_cli.py" \
  --input "tmp/qimen_input.json" \
  --output "tmp/qimen_output.json"
```

先把输入 JSON 写入可写临时目录（如仓库内 `tmp/qimen_input.json`），再执行上述命令，读取输出 JSON。不要用心算顶替脚本。
